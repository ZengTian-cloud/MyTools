...
public class RSA
{
    ...
    public string EncryptByPrivateKey(string data, string privateKey)
    {
        ...
        return Convert.ToBase64String(EncryptByPrivateKey(byteData,privateKey));
    }

    public byte[] EncryptByPrivateKey(byte[] byteData, string privateKey)
    {
        //非对称加密算法，加解密用  
    ...
    public string DecryptByPublicKey(string data, string publicKey)
    {
        ...
        byte[] byteData = Convert.FromBase64String(data);
        return Encoding_UTF8.GetString(DecryptByPublicKey(byteData,publicKey));
    }
    public byte[] DecryptByPublicKey(byte[] byteData, string publicKey)
    {
        //非对称加密算法，加解密用  
        IAsymmetricBlockCipher engine = new Pkcs1Encoding(new RsaEngine());
